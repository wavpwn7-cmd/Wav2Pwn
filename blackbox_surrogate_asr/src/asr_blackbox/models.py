...
def load_asr_bundle(model_name: str, device: str | None = None) -> ASRBundle:
    resolved_device = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
    processor = AutoProcessor.from_pretrained(model_name)
    model = AutoModelForCTC.from_pretrained(model_name).to(resolved_device)
    model.eval()
    return ASRBundle(model_name=model_name, processor=processor, model=model, device=resolved_device)
...
def load_audio(path: str, target_sr: int = 16000) -> tuple[torch.Tensor, int]:
    ...
    if waveform is None or sample_rate is None:
        try:
            ...
            backend_order = ["ffmpeg", "soundfile", "sox", None]
            if suffix == ".wav":
                backend_order = ["soundfile", "ffmpeg", "sox", None]
            ...
            for backend in backend_order:
                try:
                    ...
                    break
                except Exception as exc:  # pragma: no cover - backend availability is environment-dependent
                    backend_name = "default" if backend is None else backend
                    load_errors.append(f"{backend_name}: {exc}")

            if waveform is None or sample_rate is None:
                error_summary = " | ".join(load_errors)
                raise RuntimeError(f"Failed to load audio from {path}. Attempted loaders: {error_summary}")
