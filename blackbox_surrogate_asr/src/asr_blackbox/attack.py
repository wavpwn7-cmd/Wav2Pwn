...
def pgd_ctc_attack(
    ...
    if objective_text is None:
        raise ValueError("A target or reference transcription is required for PGD.")

    labels = _build_labels(processor, objective_text, device)
    last_loss = 0.0
    method = method.lower()

    if method not in {"pgd", "adam"}:
...
def save_waveform(path: str, waveform: torch.Tensor, sample_rate: int = 16000) -> None:
    try:
        ...
        raise RuntimeError(f"Failed to save waveform to {path}: {exc}") from exc
